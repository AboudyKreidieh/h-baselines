    ...
    try:
        while True:
            ...
            if cmd == 'step':
                ...
                remote.send(
                    CloudpickleWrapper((envs[0].observation_space,
                                        envs[0].action_space, envs[0].spec)))
            else:
...
class SubprocVecEnv(VecEnv):
    ...
    def __init__(self, env_fns, spaces=None, context='spawn', in_series=1):
        ...
        nenvs = len(env_fns)
        assert nenvs % in_series == 0, \
            "Number of envs must be divisible by number of envs to run in " \
            "series"
        self.nremotes = nenvs // in_series
        env_fns = np.array_split(env_fns, self.nremotes)
        ctx = mp.get_context(context)
        self.remotes, self.work_remotes = zip(*[ctx.Pipe()
                                                for _ in range(self.nremotes)])
