...
def nature_cnn(scaled_images, **kwargs):
    ...
    layer_2 = activ(conv(layer_1, 'c2', n_filters=64, filter_size=4, stride=2,
...
class BasePolicy(object):
    ...
    :param obs_phs: (TensorFlow Tensor, TensorFlow Tensor) a tuple containing
        an override for observation placeholder and the processed observation
    ...
    def _kwargs_check(feature_extraction, kwargs):
        ...
        :param kwargs: (dict)
        """
        # When using policy_kwargs parameter on model creation, all keywords
        # arguments must be consumed by the policy constructor except the ones
        # for the cnn_extractor network (cf nature_cnn()), where the keywords
        # arguments are not passed explicitly (using **kwargs to forward the
        # arguments) that's why there should be not kwargs left when using the
        # mlp_extractor (in that case the keywords arguments are passed
        # explicitly)
...
class FeedForwardPolicy(ActorCriticPolicy):
    ...
    :param feature_extraction: (str) The feature extraction type ("cnn" or
        "mlp")
    :param kwargs: (dict) Extra keyword arguments for the nature CNN feature
        extraction
    """
...
class MlpPolicy(FeedForwardPolicy):
    ...
    :param n_batch: (int) The number of batch to run (n_envs * n_steps)
    :param reuse: (bool) If the policy is reusable or not
    :param _kwargs: (dict) Extra keyword arguments for the nature CNN feature
    extraction
    """
