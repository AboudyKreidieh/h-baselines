...
from hbaselines.envs.efficient_hrl.create_maze_env import create_maze_env
...
flags.DEFINE_string('env', 'AntMaze',
...
class EnvWithGoal(object):
    ...
    def observation_space(self):
        if self.env_name == "AntPush":
            self.base_env.observation_space.shape = (32,)
...
def run_environment(env_name, episode_length, num_episodes):
    ...
    for ep in range(num_episodes):
        ...
        for _ in range(episode_length):
            ...
            if done:
                break
        logging.info('Episode %d reward: %.2f, Success: %d', ep + 1,
                     rewards[-1], successes[-1])
