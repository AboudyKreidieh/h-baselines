...
def get_target_updates(_vars, target_vars, tau, verbose=0):
    ...
    for var, target_var in zip(_vars, target_vars):
        ...
        soft_updates.append(tf.assign(target_var,
                                      (1. - tau) * target_var + tau * var))  # TODO: ???
...
class DDPG(OffPolicyRLModel):
    ...
    render_eval : bool
        enable rendering of the evalution environment
    memory_limit : int
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("input", reuse=False):
                    ...
                    self.actions = tf.placeholder(
                        tf.float32, shape=(None,) + self.action_space.shape,
                        name='actions')
    ...
    def action_probability(self, observation, state=None, mask=None):
        ...
        if vectorized_env:
            return self.sess.run(self.policy_tf.policy_proba,
