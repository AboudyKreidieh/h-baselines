...
parser.add_argument('-r', '--remotes', default=None,
                    help='The address of pre-existing VNC servers and '
                         'rewarders to use '
                    '(e.g. -r vnc://localhost:5900+15900,'
...
parser.add_argument('-m', '--mode', type=str, default='tmux',
                    help='tmux: run workers in a '
                         'tmux session. nohup: run workers with nohup. '
...
def new_cmd(session, name, cmd, mode, logdir, shell):
    ...
    if mode == 'tmux':
        ...
        return name, '{} >{}/{}.{}.out 2>&1 & echo ' \
                     'kill $! >>{}/kill.sh'.format(cmd,
                                                   logdir,
                                                   session,
                                                   name,
                                                   logdir)
    elif mode == 'nohup':
        return name, 'nohup {} -c {} >{}/{}.{}.out 2>&1 & echo ' \
                     'kill $! >>{}/kill.sh'.format(shell,
                                                   shlex_quote(cmd),
                                                   logdir,
                                                   session,
                                                   name,
...
def create_commands(session,
    ...
    if mode == 'tmux':
        notes += ["Use `tmux attach -t {}` to watch process output".format(
            session)]
        notes += ["Use `tmux kill-session -t {}` to kill the job".format(
            session)]
    else:
        notes += ["Use `tail -f {}/*.out` to watch process output".format(
            logdir)]
    notes += ['Point your browser to http://localhost:12345 '
              'to see Tensorboard']
    ...
    if mode == 'tmux':
        ...
        for w in windows[1:]:
            cmds += ["tmux new-window -t {} -n {} {}".format(
                session, w, shell)]
        cmds += ["sleep 1"]
...
def run():
    ...
    if args.dry_run:
        print('Dry-run mode due to -n flag, '
              'otherwise the following commands would be executed:')
    else:
